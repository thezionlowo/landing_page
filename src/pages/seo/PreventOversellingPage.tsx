import React from 'react';
import { Navbar } from '../../components/Navbar';
import { Footer } from '../../components/Footer';
import { ROUTES } from '../../lib/routes';
import {
  Sparkles,
  Layers,
  RefreshCw,
  ShoppingBag,
  Zap,
  CheckCircle2,
  Users,
  ArrowRight,
  ChevronRight,
  ShieldCheck,
  Tag,
  AlertOctagon,
  Clock,
} from 'lucide-react';

export const PreventOversellingPage: React.FC = () => {
  const breadcrumbs = [
    { name: 'Home', url: '/' },
    { name: 'Solutions', url: '/#product-powers' },
    { name: 'Prevent Overselling', url: '/solutions/prevent-overselling' },
  ];

  const faqs = [
    {
      q: 'Why do WooCommerce stores with physical shops suffer from overselling?',
      a: 'Overselling occurs because the in-store cash register and online WooCommerce store operate on disconnected inventory counts. When an in-store customer purchases an item, the website is unaware of the sale until hours later when someone manually updates it, leaving a dangerous window for online buyers to purchase the sold item.',
    },
    {
      q: 'How does ZAMERIA completely prevent stock mismatches?',
      a: 'ZAMERIA connects directly with your WooCommerce database via an instant webhook-driven sync engine. When a cashier completes a sale at the counter, the item’s WooCommerce stock count drops in under 0.5 seconds, instantly updating product availability on your website.',
    },
    {
      q: 'What if an online order is placed while a walk-in customer has the item in their hand?',
      a: 'If an online order completes first, the POS register instantly flags the product as Out of Stock or Low Stock. If the cashier rings it up first, the website updates immediately. Built-in stock protection prevents simultaneous double-allocation.',
    },
    {
      q: 'Do I need to hire an inventory manager to keep counts synchronized?',
      a: 'No. ZAMERIA’s automated bi-directional synchronization eliminates the need for manual stock audits, CSV exports, or dedicated data entry staff.',
    },
  ];

  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--canvas-bg)' }}>
      <Navbar />

      <main style={{ flex: 1, paddingTop: '130px', paddingBottom: '90px' }}>
        <div className="container" style={{ maxWidth: '1140px', margin: '0 auto', padding: '0 24px' }}>
          {/* Breadcrumbs */}
          <nav aria-label="Breadcrumb" style={{ marginBottom: '24px' }}>
            <ol style={{ display: 'flex', alignItems: 'center', gap: '8px', listStyle: 'none', fontSize: '13px', color: 'var(--text-muted)' }}>
              {breadcrumbs.map((b, i) => (
                <li key={b.url} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {i > 0 && <ChevronRight size={13} />}
                  {i === breadcrumbs.length - 1 ? (
                    <span style={{ color: 'var(--brand-navy)', fontWeight: 600 }}>{b.name}</span>
                  ) : (
                    <a href={b.url} style={{ color: 'var(--text-muted)', textDecoration: 'none' }} className="hover:text-primary">
                      {b.name}
                    </a>
                  )}
                </li>
              ))}
            </ol>
          </nav>

          {/* Hero */}
          <div style={{ textAlign: 'center', maxWidth: '880px', margin: '0 auto 64px' }}>
            <div className="eyebrow-badge purple" style={{ margin: '0 auto 20px' }}>
              <AlertOctagon size={13} style={{ marginRight: '6px' }} />
              <span>SOLVING MULTI-CHANNEL STOCK CONFLICTS</span>
            </div>
            <h1 style={{ fontSize: 'clamp(32px, 4.5vw, 52px)', fontWeight: 800, color: 'var(--brand-navy)', letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: '24px' }}>
              How to Stop Overselling on WooCommerce When You Have a Physical Store
            </h1>
            <p style={{ fontSize: '18px', color: 'var(--text-body)', lineHeight: 1.6, marginBottom: '36px', maxWidth: '720px', margin: '0 auto 36px' }}>
              Eliminate the frustration of customer refunds and apology emails. Connect your physical counter with WooCommerce to ensure online and offline stock always agree.
            </p>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', flexWrap: 'wrap' }}>
              <a href={ROUTES.trial} className="btn btn-hero-gradient" style={{ padding: '15px 36px', fontSize: '15px' }}>
                <span>Start 7-Day Free Trial</span>
                <ArrowRight size={15} />
              </a>
              <a href="/woocommerce-inventory-sync" className="btn btn-secondary" style={{ padding: '15px 28px', fontSize: '15px' }}>
                <span>Explore Inventory Sync</span>
              </a>
            </div>
          </div>

          {/* The 3 Steps to Eliminate Overselling */}
          <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-subtle)', borderRadius: '24px', padding: '44px 36px', marginBottom: '80px', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ textAlign: 'center', maxWidth: '680px', margin: '0 auto 40px' }}>
              <h2 style={{ fontSize: '28px', fontWeight: 800, color: 'var(--brand-navy)', letterSpacing: '-0.02em', marginBottom: '12px' }}>
                The 3-Step Strategy to Zero Stock Mismatches
              </h2>
              <p style={{ fontSize: '15px', color: 'var(--text-body)', lineHeight: 1.6 }}>
                How connected retailers replace manual spreadsheets with automated real-time retail synchronization.
              </p>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
              <div style={{ backgroundColor: 'var(--canvas-bg)', border: '1px solid var(--border-subtle)', borderRadius: '18px', padding: '28px' }}>
                <div style={{ fontSize: '28px', fontWeight: 900, color: '#2563eb', fontFamily: 'var(--font-mono)', marginBottom: '12px' }}>01</div>
                <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--brand-navy)', marginBottom: '10px' }}>Single Source of Truth</h3>
                <p style={{ fontSize: '14px', color: 'var(--text-body)', lineHeight: 1.6 }}>
                  Stop maintaining duplicate product spreadsheets. Use your WooCommerce product database as the master catalog for descriptions, barcodes, and inventory.
                </p>
              </div>

              <div style={{ backgroundColor: 'var(--canvas-bg)', border: '1px solid var(--border-subtle)', borderRadius: '18px', padding: '28px' }}>
                <div style={{ fontSize: '28px', fontWeight: 900, color: '#10b981', fontFamily: 'var(--font-mono)', marginBottom: '12px' }}>02</div>
                <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--brand-navy)', marginBottom: '10px' }}>Sub-Second Counter Sync</h3>
                <p style={{ fontSize: '14px', color: 'var(--text-body)', lineHeight: 1.6 }}>
                  Replace disconnected cash registers with ZAMERIA POS. When an item is scanned and paid for at your physical counter, website stock drops immediately.
                </p>
              </div>

              <div style={{ backgroundColor: 'var(--canvas-bg)', border: '1px solid var(--border-subtle)', borderRadius: '18px', padding: '28px' }}>
                <div style={{ fontSize: '28px', fontWeight: 900, color: '#9333ea', fontFamily: 'var(--font-mono)', marginBottom: '12px' }}>03</div>
                <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--brand-navy)', marginBottom: '10px' }}>Automatic Out-of-Stock Protection</h3>
                <p style={{ fontSize: '14px', color: 'var(--text-body)', lineHeight: 1.6 }}>
                  When the final unit of any product (or specific size/color variation) sells, both WooCommerce and POS register status update to Out of Stock in real time.
                </p>
              </div>
            </div>
          </div>

          {/* Related Solutions Navigation */}
          <div style={{ backgroundColor: 'var(--surface-white)', border: '1px solid var(--border-subtle)', borderRadius: '20px', padding: '36px', marginBottom: '80px' }}>
            <h3 style={{ fontSize: '20px', fontWeight: 800, color: 'var(--brand-navy)', marginBottom: '20px' }}>Explore Related Solutions</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
              <a href="/woocommerce-inventory-sync" style={{ padding: '16px', borderRadius: '12px', backgroundColor: 'var(--canvas-bg)', border: '1px solid var(--border-subtle)', display: 'block', textDecoration: 'none' }}>
                <div style={{ fontWeight: 700, color: 'var(--brand-navy)', fontSize: '14px', marginBottom: '4px' }}>Inventory Synchronization →</div>
                <div style={{ fontSize: '12.5px', color: 'var(--text-muted)' }}>Learn how bi-directional inventory syncing works.</div>
              </a>
              <a href="/woocommerce-pos" style={{ padding: '16px', borderRadius: '12px', backgroundColor: 'var(--canvas-bg)', border: '1px solid var(--border-subtle)', display: 'block', textDecoration: 'none' }}>
                <div style={{ fontWeight: 700, color: 'var(--brand-navy)', fontSize: '14px', marginBottom: '4px' }}>WooCommerce POS System →</div>
                <div style={{ fontSize: '12.5px', color: 'var(--text-muted)' }}>High-speed register features and hardware support.</div>
              </a>
              <a href="/woocommerce-pos-nigeria" style={{ padding: '16px', borderRadius: '12px', backgroundColor: 'var(--canvas-bg)', border: '1px solid var(--border-subtle)', display: 'block', textDecoration: 'none' }}>
                <div style={{ fontWeight: 700, color: 'var(--brand-navy)', fontSize: '14px', marginBottom: '4px' }}>Retail POS in Nigeria →</div>
                <div style={{ fontSize: '12.5px', color: 'var(--text-muted)' }}>Tailored for Nigerian multi-channel retail businesses.</div>
              </a>
              <a href="/industries/fashion" style={{ padding: '16px', borderRadius: '12px', backgroundColor: 'var(--canvas-bg)', border: '1px solid var(--border-subtle)', display: 'block', textDecoration: 'none' }}>
                <div style={{ fontWeight: 700, color: 'var(--brand-navy)', fontSize: '14px', marginBottom: '4px' }}>Fashion & Boutique POS →</div>
                <div style={{ fontSize: '12.5px', color: 'var(--text-muted)' }}>Size, color & variation matrix counter selling.</div>
              </a>
            </div>
          </div>

          {/* FAQs */}
          <div style={{ maxWidth: '800px', margin: '0 auto 80px' }}>
            <h2 style={{ fontSize: '28px', fontWeight: 800, color: 'var(--brand-navy)', textAlign: 'center', marginBottom: '32px' }}>
              Preventing Overselling FAQs
            </h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {faqs.map((f, i) => (
                <div key={i} style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-subtle)', borderRadius: '16px', padding: '24px' }}>
                  <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--brand-navy)', marginBottom: '8px' }}>{f.q}</h3>
                  <p style={{ fontSize: '14.5px', color: 'var(--text-body)', lineHeight: 1.6, margin: 0 }}>{f.a}</p>
                </div>
              ))}
            </div>
          </div>

          {/* CTA */}
          <div style={{ backgroundColor: 'var(--brand-navy)', borderRadius: '28px', padding: '56px 40px', textAlign: 'center', color: '#ffffff', boxShadow: '0 20px 40px rgba(7, 26, 49, 0.25)' }}>
            <h2 style={{ fontSize: '32px', fontWeight: 800, color: '#ffffff', marginBottom: '16px' }}>
              Never Oversell on WooCommerce Again
            </h2>
            <p style={{ fontSize: '16px', color: '#cbd5e1', maxWidth: '620px', margin: '0 auto 32px' }}>
              Protect your brand reputation, delight your customers, and run your retail business with complete peace of mind.
            </p>
            <a href={ROUTES.trial} className="btn btn-hero-gradient" style={{ padding: '16px 40px', fontSize: '16px', display: 'inline-flex' }}>
              <span>Start 7-Day Free Trial</span>
              <ArrowRight size={16} />
            </a>
          </div>
        </div>
      </main>

      <Footer />
    </div>
  );
};
