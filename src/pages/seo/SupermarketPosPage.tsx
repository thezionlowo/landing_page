...
export const SupermarketPosPage: React.FC = () => {
  const breadcrumbs = [
    ...
    { name: 'Supermarkets & Grocery', url: '/industries/supermarkets' },
  ];

  const faqs = [
    {
      q: 'Can ZAMERIA handle high-volume barcode scanning for busy grocery stores?',
      a: 'Yes. ZAMERIA is engineered for ultra-fast keyboard and laser barcode scanner inputs. Items are added to the active cart in milliseconds without keystroke delays or search lag, keeping checkout lines moving quickly.',
    },
    {
      q: 'How does ZAMERIA manage thousands of fast-moving grocery SKUs?',
      a: 'ZAMERIA utilizes high-speed local IndexedDB caching on the POS register. Your entire grocery catalog is stored and searchable locally on the counter computer, ensuring instantaneous product lookup even with 15,000+ SKUs.',
    },
    {
      q: 'Can cashiers hold a cart when a customer goes back to pick an extra item?',
      a: 'Yes. The POS includes a multi-cart Hold & Resume feature. Cashiers can place a customer’s basket on hold, serve the next shopper in line, and resume the held order with a single click.',
    },
    {
      q: 'Does ZAMERIA support thermal receipt printing and automated cash drawer triggers?',
      a: 'Yes. Standard 80mm thermal receipt printers print itemized breakdowns, cashier names, VAT/tax details, and trigger cash drawers automatically via standard RJ11 kick connections.',
    },
  ];
      ...
      <main style={{ flex: 1, paddingTop: '130px', paddingBottom: '90px' }}>
        <div className="container" style={{ maxWidth: '1140px', margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ textAlign: 'center', maxWidth: '880px', margin: '0 auto 64px' }}>
            <div className="eyebrow-badge green" style={{ margin: '0 auto 20px' }}>
              <ShoppingCart size={13} style={{ marginRight: '6px' }} />
              <span>SUPERMARKET & GROCERY RETAIL</span>
            </div>
            <h1 style={{ fontSize: 'clamp(32px, 4.5vw, 52px)', fontWeight: 800, color: 'var(--brand-navy)', letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: '24px' }}>
              WooCommerce POS & Inventory Management for Supermarkets & Grocery Stores
            </h1>
            <p style={{ fontSize: '18px', color: 'var(--text-body)', lineHeight: 1.6, marginBottom: '36px', maxWidth: '720px', margin: '0 auto 36px' }}>
              High-speed barcode scanning, multi-register support, cart holding, and live inventory synchronization between your supermarket checkout lines and your online grocery store.
            </p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px', marginBottom: '80px' }}>
            <div className="glass-card" style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-subtle)', borderRadius: '20px', padding: '32px 26px' }}>
              ...
              <p style={{ fontSize: '14px', color: 'var(--text-body)', lineHeight: 1.6 }}>
                Scan continuous barcodes effortlessly. Handles packaged foods, beverages, toiletries, and household goods with instant sub-millisecond cart feedback.
              </p>
            </div>
            ...
            <div className="glass-card" style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-subtle)', borderRadius: '20px', padding: '32px 26px' }}>
              ...
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--brand-navy)', marginBottom: '10px' }}>Cart Hold & Resume</h2>
              <p style={{ fontSize: '14px', color: 'var(--text-body)', lineHeight: 1.6 }}>
                Never hold up the queue. Cashiers can park an order when a customer retrieves an extra item and immediately ring up the next shopper.
              </p>
            </div>
            ...
            <div className="glass-card" style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-subtle)', borderRadius: '20px', padding: '32px 26px' }}>
              ...
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--brand-navy)', marginBottom: '10px' }}>80mm Thermal Receipts</h2>
              <p style={{ fontSize: '14px', color: 'var(--text-body)', lineHeight: 1.6 }}>
                High-density thermal printing with clean itemized lines, discounts, payment method breakdown (Cash/POS Card/Transfer), and custom footer notes.
              </p>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--border-subtle)', borderRadius: '24px', padding: '48px 36px', marginBottom: '80px' }}>
            <h2 style={{ fontSize: '26px', fontWeight: 800, color: 'var(--brand-navy)', marginBottom: '16px' }}>
              Built for Fast-Paced Supermarket Counters & Online Delivery
            </h2>
            <p style={{ fontSize: '15px', color: 'var(--text-body)', lineHeight: 1.7, marginBottom: '28px' }}>
              Managing a physical grocery store alongside an online delivery catalog usually results in chaotic inventory discrepancies. ZAMERIA bridges this gap with unified product catalogs and automated stock synchronization.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                <CheckCircle2 size={18} style={{ color: '#16a34a', flexShrink: 0, marginTop: '3px' }} />
                <span style={{ fontSize: '14px', color: 'var(--brand-navy)', fontWeight: 600 }}>Multi-cashier logins with individual shift accountability</span>
              </div>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                <CheckCircle2 size={18} style={{ color: '#16a34a', flexShrink: 0, marginTop: '3px' }} />
                <span style={{ fontSize: '14px', color: 'var(--brand-navy)', fontWeight: 600 }}>Offline checkout support when store internet fluctuates</span>
              </div>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                <CheckCircle2 size={18} style={{ color: '#16a34a', flexShrink: 0, marginTop: '3px' }} />
                <span style={{ fontSize: '14px', color: 'var(--brand-navy)', fontWeight: 600 }}>Real-time stock deduction prevents online grocery overselling</span>
              </div>
              <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                <CheckCircle2 size={18} style={{ color: '#16a34a', flexShrink: 0, marginTop: '3px' }} />
                <span style={{ fontSize: '14px', color: 'var(--brand-navy)', fontWeight: 600 }}>Works on existing Windows laptops, MacBooks, and POS PCs</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-card" style={{ backgroundColor: '#071A31', borderRadius: '24px', padding: '48px 36px', textAlign: 'center', color: '#ffffff' }}>
            <h2 style={{ fontSize: '28px', fontWeight: 800, marginBottom: '14px', color: '#ffffff' }}>
              Upgrade Your Supermarket Checkout & Online Stock Sync
            </h2>
            <p style={{ fontSize: '15px', color: '#94a3b8', maxWidth: '600px', margin: '0 auto 28px', lineHeight: 1.6 }}>
              Connect your WooCommerce grocery store to fast in-store checkout counters today.
            </p>
